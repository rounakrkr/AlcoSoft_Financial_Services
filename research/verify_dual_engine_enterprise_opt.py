...
# Set STRATEGY_SETS_PATH before loading the strategy sets
...
from research.strategy_sets_opt import load_strategy_sets, StrategySetDefinition
...
class LongEngineConfig:
    ...
    market_gap_threshold: float = 0.006       # >= 0.6% Gap Up
    market_breadth_requirement: float = 0.30  # >= 30% of universe
    exclude_gap_threshold: float = -0.008     # Exclude any stock with <= -0.8% Gap Down
    rsi_exit_threshold: float = 76.0          # Exit if RSI(14) >= 76.0
    profit_target_pct: float = 0.010          # Book partial at +1.0%
    partial_booking_fraction: float = 1.00    # 100% qty to cover
    stop_loss_pct: float = 0.009              # Flat 0.9% SL

@dataclass(frozen=True)
class ShortEngineConfig:
    """Short Engine Execution Parameters"""
    market_gap_threshold: float = -0.006      # <= -0.6% Gap Down
    market_breadth_requirement: float = 0.50  # >= 50% of universe
    target_gap_threshold: float = -0.008      # Target stocks with <= -0.8% Gap Down
    rsi_exit_threshold: float = 22.0          # Exit if RSI(16) <= 22.0
...
class MarketRegimeAnalyzer:
    ...
    def get_bull_days(self, config: LongEngineConfig) -> Set[datetime.date]:
        ...
        for curr_d in self.trading_dates:
            gaps = self.gaps_by_date.get(curr_d, [])
            if not gaps:
                continue
...
class SignalGenerator:
    ...
    def precompute_signals(self, start_date=None):
        logger.info("Precomputing Entry Signals via Vectorized Pandas...")
        target_start = None
        if start_date:
        ...
        for sym, df in self.stock_dfs.items():
            n = len(df)
            self.long_signals[sym] = [False] * n
            self.short_signals[sym] = [False] * n
            ...
            high_10_max = df["high"].shift(1).rolling(10).max()
            low_10_min = df["low"].shift(1).rolling(10).min()
            ...
            short_mask = (
                ...
                (close_0 < low_10_min) &
                (close_0 <= high_1) &
                (close_0 >= vwap_0 * 0.988)
            )
            
            # Apply start date filtering
            if target_start:
                start_mask = df.index.date >= target_start
                long_mask = long_mask & start_mask
                short_mask = short_mask & start_mask
                
            # Set the first 11 values to False
            long_mask.iloc[:11] = False
            short_mask.iloc[:11] = False
            
            # Fill NaN with False
            long_mask = long_mask.fillna(False)
            short_mask = short_mask.fillna(False)
            
            self.long_signals[sym] = long_mask.tolist()
            self.short_signals[sym] = short_mask.tolist()

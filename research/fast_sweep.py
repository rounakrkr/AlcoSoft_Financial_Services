...
def update_leaderboard(entries, filename="results/midcap50_top10_leaderboard.txt"):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for i, e in enumerate(top10):
            s = e['stats']
            f.write(f"  # {i+1:<2} | {e['name']:<18} | {e['params']:<45} | WR={s['wr']:<5.2f}% | T={s['trades']:<4} | Gross={s['gross']:<6.2f}% | STT={s['stt']:<6.2f}% | NET={s['net']:<6.2f}% | Exp=₹{s['expectancy']:<6.2f} per trade | Diff={e['diff']}\\n")
        f.write("="*140 + "\\n")
...
def run_sweeps():
    ...
    print(f"BASE_DYN_OFF Net: {res['net']:.2f}%")
    ...
    print(f"NO_SHORTS Net: {res['net']:.2f}%")
    ...
    print(f"SAVIOR_EXIT Net: {res['net']:.2f}%")
    ...
    print(f"TIGHT_GAP Net: {res['net']:.2f}%")
    
    update_leaderboard(all_results)
    
    print("\\nPhase B: Sweeping Long Engine Parameters (with shorts disabled)")
    ...
    for sl in sls:
        for pt in pts:
            for rsi in rsis:
                ...
                all_results.append({
                    'name': f'L_SWP_SL{sl}_PT{pt}_RSI{rsi}',

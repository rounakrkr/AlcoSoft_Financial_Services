...
for i in range(1, len(dates)):
    ...
    strong = (gaps >= 0.005).sum() # GAP UP 0.5% BASELINE
    total = len(gaps)
    if total > 0 and (strong / total) >= 0.40:
        daily_gaps.append(curr_d)

strong40_up_days = set(daily_gaps)

def run_long_live_simulation():
    ...
    for ts in timeline:
        ...
        # 1. Manage open positions (Long Sell)
        for sym in list(positions.keys()):
            ...
            if not pos.get("partial_done", False):
                profit_now = (close - ep) / ep # profit when price rises
                if profit_now >= PROFIT_TARGET:
                    cover_qty = max(1, int(qty * PARTIAL_FRAC))
                    if cover_qty >= qty: cover_qty = max(0, qty - 1)
                    if cover_qty > 0:
                        trades.append({"pnl": (close - ep) * cover_qty, "reason": "PARTIAL_PROFIT", "ex": close, "qty": cover_qty})
                        pos["qty"] -= cover_qty
                        qty = pos["qty"]
            ...
            if rsi0 >= RSI_EXIT_THR:
                trades.append({"pnl": (close - ep) * qty, "reason": "RSI_EXIT", "ex": close, "qty": qty})
            ...
            if low <= pos["sl"]: 
                ...
                cond = evaluator._evaluate_conditions(cover_set_def, ctx)
        ...
        # 2. Look for new entries (Longs)
        if len(positions) >= MP: continue
        if ts.hour >= 15: continue
        if d not in strong40_up_days: continue
        ...
        for sym, df in stock_dfs.items():
            ...
            # EXCLUDE STOCKS THAT GAPPED DOWN -0.8% OR MORE (Leave them for the Short Strategy)
            gap_pct = all_gaps.get((d, sym), 0.0)
            if gap_pct <= -0.008: continue
            ...
            if cond and all(r.get("fired") for r in cond):
                ...
                cover_cond = evaluator._evaluate_conditions(cover_set_def, cover_ctx)
                ...
                if idx+1 < len(df):
                    ...
                    qty = int(per_slot // ep)
                    if qty > 0:
                        sl_p = round_to_tick(ep * (1 - SL_PCT)) # SL is BELOW entry for longs
...
print("Running Long Live Configuration Simulation (Baseline)...")
print(f"Config: MP={MP}, GapUp40=True, Partial={PARTIAL_FRAC*100}% @ {PROFIT_TARGET*100}%, RSI_Exit={RSI_EXIT_THR}")

trades = run_long_live_simulation()
...
if not df_t.empty:
    ...
    print(f"Est. STT:     {total_stt/CAPITAL*100:.1f}%")

...
# Baseline Market Gap: 40% of stocks gap down > 0.8%
# Baseline Individual Gap: Only short if stock gaps down > 0.8%
IND_GAP_BASE = -0.008
all_gaps = {}
daily_gaps = []
...
RSI_LAGS = [0, 1] 
RSI_THRESHOLDS = list(range(20, 51)) # 20 to 50
...
for sym, df in stock_dfs.items():
    for idx in range(10, len(df)):
        ...
        if cond and all(r.get("fired") for r in cond):
            ...
            if not prev_high_break(c_ctx)["fired"]:
                precomputed_entries[sym][idx] = True

def run_rsi_sweep(rsi_len, rsi_lag, rsi_thr):
    ...
    for ts in timeline:
        ...
        for sym in list(positions.keys()):
            ...
            if not pos.get("partial_done", False):
                if (ep - close) / ep >= PROFIT_TARGET:
                    ...
                    if cover_qty > 0:
                        trades.append({"pnl": (ep - close) * cover_qty, "ep": ep, "qty": cover_qty, "reason": "PARTIAL_PROFIT"})
                        pos["qty"] -= cover_qty; qty = pos["qty"]
                        pos["partial_done"] = True
                        if qty <= 0: closed.append(sym); continue
            
            if rsi_val <= rsi_thr:
                trades.append({"pnl": (ep - close) * qty, "ep": ep, "qty": qty, "reason": "RSI_EXIT"})
            ...
            if high >= pos["sl"]:
                ...
                if prev_high_break(ctx)["fired"]:
                    if idx+1 < len(df): 
                        ...
                        reason = "DYN_EXIT"
            if ex:
                trades.append({"pnl": (ep - ex) * qty, "ep": ep, "qty": qty, "reason": reason})
                closed.append(sym)
        ...
        for sym, df in stock_dfs.items():
            ...
            # Baseline filter: stock must individually gap down > 0.5%
    ...
    for sym, pos in positions.items():
        ...
        trades.append({"pnl": (pos["ep"] - lc) * pos["qty"], "ep": pos["ep"], "qty": pos["qty"], "reason": "EOD"})
    
    return trades

results = []
print(f"Starting Massive RSI Sweep (Total tests: {len(RSI_LENGTHS) * len(RSI_LAGS) * len(RSI_THRESHOLDS)})...")
import time
start_t = time.time()
counter = 0
for rsi_len in RSI_LENGTHS:
    for rsi_lag in RSI_LAGS:
        for rsi_thr in RSI_THRESHOLDS:
            ...
            trades = run_rsi_sweep(rsi_len, rsi_lag, rsi_thr)
            counter += 1
            if counter % 50 == 0:
            ...
            # Use STT = 0.035% of selling amount
...
res_df = pd.DataFrame(results).sort_values("Net_Pct", ascending=False)
print("\n" + "="*80)
print("MASSIVE RSI SWEEP RESULTS (Top 20, Sorted by Net Return)")
print("="*80)

...
# Sweep Configuration
RSI_LENGTHS = [14, 21]
RSI_LAGS = [0, 1] # 0 for live, 1 for previous candle
RSI_THRESHOLDS = [20, 25, 28, 30, 32, 35, 40, 45, 50]

def run_rsi_sweep(rsi_len, rsi_lag, rsi_thr):
    per_slot = BUYING_POWER / MP
    positions = {}; trades = []
    
    # Calculate custom RSI if needed (14 is default in cache, so we might need to compute 21 if it's not there)
    # The cache actually has "rsi" which is rsi(14). Let's stick to the cached RSI or compute it.
    # Actually, calculating RSI inside the loop is slow. Let's just use the default 'rsi' (which is 14) 
    # to avoid extreme slowness, but test lag0 vs lag1 and different thresholds.
    # To test length, we would need to precompute it. Let's precompute rsi_21 for all stocks.
    
    # Precompute custom RSI
    rsi_col = "rsi" if rsi_len == 14 else f"rsi_{rsi_len}"
    if rsi_len != 14:
        for sym, df in stock_dfs.items():
    ...
    for ts in timeline:
        ...
        for sym in list(positions.keys()):
            ...
            if high >= pos["sl"]:
                ...
                if prev_high_break(ctx)["fired"]: # USING THE #1 WINNER DYNAMIC EXIT
                    if idx+1 < len(df): 
...
results = []
print("Starting RSI Sweet Spot Sweep...")
...
print("\n" + "="*80)
print("RSI SWEEP RESULTS (Sorted by Gross Return)")
print("="*80)

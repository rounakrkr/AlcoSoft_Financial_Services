...
def run_short_engine(mkt_thresh=-0.008, mkt_breadth=0.40, indiv_gap=-0.008, rsi_exit=15.0):
    bear_days = set()
    for i in range(1, len(dates)):
        curr_d = dates[i]
        gaps = [g for (d, s), g in all_daily_gaps.items() if d == curr_d]
        if not gaps: continue
        if sum(1 for g in gaps if g <= mkt_thresh) / len(gaps) >= mkt_breadth:
    ...
    for ts in timeline:
        ...
        for sym, pos in positions.items():
            ...
            if idx >= 1:
                ...
                if c0 > h1:
                    ...
                    trades.append({"sym": sym, "pnl": (pos["ep"] - ex_p) * pos["qty"], "qty": pos["qty"], "ex": ex_p, "type": "SHORT_DYN_SL_LAG0"})
                    syms_to_close.append(sym); continue
        ...
        for sym, df in stock_dfs.items():
            ...
            if all_daily_gaps.get((current_date, sym), 0.0) > indiv_gap: continue # ONLY GAP DOWN <= -0.8%
...
def sweep_axis(axis_name, values, param_dict, file_obj):
    file_obj.write(f"\n### Sweeping Axis: `{axis_name}`\n")
    file_obj.write("| Value | Trades | Win Rate | Gross | STT | Net Return |\n")
    file_obj.write("| :--- | :--- | :--- | :--- | :--- | :--- |\n")
    ...
    for val in values:
        ...
        trades = run_short_engine(**kwargs)
        df_t = pd.DataFrame(trades)
        if df_t.empty:
            file_obj.write(f"| {val} | 0 | 0.0% | 0.00% | 0.00% | 0.00% |\n")
            continue
        ...
        if n_pct >= 6.0: best = "🟩 (Holy Grail)"
        elif n_pct >= 3.0: best = "🟨 (Good)"
        elif n_pct >= 0.0: best = "⬜ (Break Even)"
        else: best = "🟥 (Poor)"
            
        file_obj.write(f"| {val} | {tot} | {wr:.1f}% | +{gross/CAPITAL*100:.2f}% | -{stt/CAPITAL*100:.2f}% | **{n_pct:+.2f}%** {best} |\n")
        file_obj.flush()

with open("C:/Users/RounakKR/.gemini/antigravity/brain/acbe5f97-2544-4468-b175-d002090fd989/axis_report.md", "w", encoding="utf-8") as f:
    f.write("# 🚀 SHORT SELLING ENGINE: PARAMETER AXIS SWEEP REPORT 🚀\n")

    BASE = {"mkt_thresh": -0.008, "mkt_breadth": 0.40, "indiv_gap": -0.008, "rsi_exit": 15.0}

    sweep_axis("mkt_thresh", [-0.004, -0.006, -0.008, -0.010], BASE, f)
    sweep_axis("mkt_breadth", [0.30, 0.40, 0.50], BASE, f)
    sweep_axis("indiv_gap", [-0.006, -0.008, -0.010], BASE, f)
    sweep_axis("rsi_exit", [12.0, 15.0, 18.0], BASE, f)
